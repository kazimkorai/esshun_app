...
class EditProfileScreenState extends State<EditProfileScreen> {
  ...
  await  getUserDetail(userId: int.parse(sharedPref.getString(USER_ID).toString())).then((value) {

      toast(value.data!.email.toString());
      ...
      selectGender = value.data!.gender.toString()=='null'?"MALE":value.data!.gender.toString().validate();
      addressController.text = value.data!.address.validate();
      contactNumberController.text = value.data!.contactNumber.validate();

      appStoreClient.setUserEmail(value.data!.email.validate());
      appStoreClient.setUserName(value.data!.username.validate());
      appStoreClient.setFirstName(value.data!.firstName.validate());
  ...
  Widget profileImage() {
    if (imageProfile != null) {
     ...
     // return Text("no image");
      if (sharedPref.getString(USER_PROFILE_PHOTO).toString()!='null') {
        return Center(
          child: ClipRRect(
            borderRadius: BorderRadius.circular(100),
            child: commonCachedNetworkImage(appStoreClient.userProfile.validate(), fit: BoxFit.cover, height: 100, width: 100),
          ),
        );
      } else {
        return Center(
  ...
  Future<void> saveProfile() async {
    appStoreClient.setLoading(true);
    String token=await sharedPref.getString(TOKEN).toString();
    await updateProfile(
      token: token,
      isRegister: false,
                    ...
                    children: [
                      ...
                      Align(
                        ...
                        child: Container(
                          ...
                          height: 35,
                          width: 35,
                          decoration: BoxDecoration(borderRadius: BorderRadius.circular(30), color: primaryColorD),
                          child: IconButton(
                            onPressed: () {
                              getImage();
                            },
