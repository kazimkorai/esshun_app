...
class ChatMessageServiceClient extends BaseService {
  FirebaseFirestore fireStore = FirebaseFirestore.instance;
  late CollectionReference userRef;
  FirebaseStorage _storage = FirebaseStorage.instance;

  ChatMessageServiceClient() {
  ...
  Future<DocumentReference> addMessage(ChatMessageModelClient data) async {
    var doc = await ref!.doc(data.senderId).collection(data.receiverId!).add(data.toJson());
    doc.update({'id': doc.id});
    return doc;
  }

  Future<void> addMessageToDb(DocumentReference senderDoc, ChatMessageModelClient data, UserModel sender, UserModel? user, {File? image}) async {
    String imageUrl = '';
