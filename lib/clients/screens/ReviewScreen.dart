...
class ReviewScreenState extends State<ReviewScreen> {
  ...
  void init() async {
    mqttForUser();
    appStoreClient.walletPresetTipAmount.isNotEmpty ? appStoreClient.setWalletTipAmount(appStoreClient.walletPresetTipAmount) : appStoreClient.setWalletTipAmount('10|20|50');
  }
                ...
                children: [
                  ...
                  if (widget.rideRequest.paymentStatus != PAID && isTipShow)
                    Wrap(
                      spacing: 10,
                      runSpacing: 16,
                      children: appStoreClient.walletPresetTipAmount.split('|').map((e) {
                        return inkWellWidget(
                          onTap: () {
                            currentIndex = appStoreClient.walletPresetTipAmount.split('|').indexOf(e);
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                                color: currentIndex == appStoreClient.walletPresetTipAmount.split('|').indexOf(e) ? primaryColor : primaryColor.withOpacity(0.4),
                                borderRadius: BorderRadius.circular(defaultRadius)),
                            child: Text(appStoreClient.currencyPosition == LEFT ? '${appStoreClient.currencyCode} $e' : '$e ${appStoreClient.currencyCode}', style: primaryTextStyle(color: Colors.white,size: 14)),
                          ),
                        );
                      }).toList(),
                    ),
