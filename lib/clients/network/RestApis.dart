...
import 'package:taxi_driver/clients/screens/RegisterScreen.dart';
...
import '../model/OtpVerifiedModelClient.dart';
import '../model/WalletInfoModel.dart';
import '../screens/PhoneOTPVerificationClient.dart';
...
import '../screens/LoginScreenClient.dart';
import '../utils/Constants.dart';
import '../utils/Extensions/app_common.dart';
import 'NetworkUtils.dart';

Future<LoginResponse> upDateProfileApi(Map request) async {

  Response response = await buildHttpResponse('update-profile',
...
Future<void> loginWithOtp(Map request, BuildContext context,bool isDriver) async {

  if(isDriver){
    Navigator.of(context).push(
        MaterialPageRoute(builder: (context) => OTPVerificationDriver(request['contact_number'].toString()),)
    );
    var response = await buildHttpResponse('send-otp', request: request, method: HttpMethod.POST);
    var otpModel= OtpSendModel.fromJson(jsonDecode(response.body));
    showLocalNotification(otpModel.message.toString(),otpModel.otp.toString());
    print(response);
  }
    ...
    Navigator.of(context).push(
        MaterialPageRoute(builder: (context) => OTPVerificationPageClient(request['contact_number'].toString()),)
    );
    var response = await buildHttpResponse('send-otp', request: request, method: HttpMethod.POST);
    var otpModel= OtpSendModel.fromJson(jsonDecode(response.body));
    showLocalNotification(otpModel.message.toString(),otpModel.otp.toString());
...
Future<void> otpVerify(Map request, BuildContext context, bool isDriver) async {

  var response = await buildHttpResponse('verify-otp', request: request, method: HttpMethod.POST);
  var otpModel= OtpVerifiedModel.fromJson(jsonDecode(response.body));
  showLocalNotification(otpModel.message.toString(),"");
  print(otpModel.user.contactNumber.toString());
  print(otpModel.user.userType.toString());
  print(otpModel.token.toString()+"*token setting");
  sharedPref.setString(TOKEN,otpModel.token.toString());
  print( sharedPref.getString(TOKEN).toString()+"*token setting");
  sharedPref.setString(UID, otpModel.user.id.toString());
  sharedPref.setString(USER_ID, otpModel.user.id.toString());

  if(isDriver){

    if(otpModel.user.firstName.toString()=="null"){
      Navigator.push(
        context,
        MaterialPageRoute(builder: (context) =>  DriverRegisterScreen(token: otpModel.token,userId: otpModel.user.id.toString(),)),
      );
    }
    else{
      Navigator.push(
    ...
    if(otpModel.user.firstName.toString()=="null"){
      Navigator.push(
        context,
        MaterialPageRoute(builder: (context) =>  RegisterScreen(userId: otpModel.user.id.toString(),userName: otpModel.user.username, token: otpModel.token,contactNo: otpModel.user.contactNumber.toString(),)),
      );
    }
    else{
      Navigator.push(
...
Future updateProfile(
  ...
  multiPartRequest.fields['id'] = userId;
...
Future<LoginResponse> getUserDetailClient({int? userId}) async {
...
Future<CurrentRequestModel> getCurrentRideRequestClient() async {
...
Future<NearByDriverModel> getNearByDriverList({LatLng? latLng}) async {
  print('near-by-driver'+latLng!.latitude.toString());
